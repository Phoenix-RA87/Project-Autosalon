...
//procedure TRAKa.vibor(Sender: TObject);  // ДОДЕЛАТЬ !!!  Процедура выбора для фильтрации данных в ДБГрид.
//begin
...
writeln(f1,'<th>'+'Марка'+'</th>');
writeln(f1,'<th>'+'Модель'+'</th>');
writeln(f1,'<th>'+'Мощность'+'</th>');
writeln(f1,'<th>'+'Объём'+'</th>');
writeln(f1,'<th>'+'РасходТоплива'+'</th>');
writeln(f1,'<th>'+'ТипКоробки'+'</th>');
writeln(f1,'<th>'+'Производ'+'</th>');
writeln(f1,'<th>'+'БазЦена'+'</th>');
writeln(f1,'<th>'+'Кол-во'+'</th>');
...
procedure TRAKa.BitBtn2Click(Sender: TObject);
 ...
 DBGrid1.Columns.Items[0].Title.Caption :='КодТовара';
  DBGrid1.Columns.Items[1].Title.Caption :='Марка';
   DBGrid1.Columns.Items[2].Title.Caption :='Модель';
     DBGrid1.Columns.Items[3].Title.Caption :='ДатаПоставки';
      DBGrid1.Columns.Items[4].Title.Caption :='№ Сертиф';
       DBGrid1.Columns.Items[5].Title.Caption :='БазЦена';
        DBGrid1.Columns.Items[6].Title.Caption :='Мощность';
         DBGrid1.Columns.Items[7].Title.Caption :='№ Пост';
          DBGrid1.Columns.Items[8].Title.Caption :='РасходТопливо';
           DBGrid1.Columns.Items[9].Title.Caption :='ТипКаробки';
            DBGrid1.Columns.Items[10].Title.Caption :='Производитель';
             DBGrid1.Columns.Items[11].Title.Caption :='Объём';

//  if not DBGrid1.DataSource.DataSet.Locate('Marka', Edit1.Text, [loCaseInsensitive, loPartialKey]) then
//    ShowMessage('нету такого')
//  else ShowMessage('НАШЛИ');
end;
...
OE1.Cells[1,1].Value:='Каталог автомобилей'+
'На дату'+' '+'Дата'+' '+DateToStr(DateTimePicker1.Date);
OE1.Cells[2,1].Value:='Код товара';
OE1.Cells[2,2].value:='Марка';
OE1.Cells[2,3].Value:='Модель';
OE1.Cells[2,4].Value:='ДатаПоставки';
OE1.Cells[2,5].Value:='№ Сертиф';
OE1.Cells[2,6].Value:='БазЦена';
OE1.Cells[2,7].Value:='Мощность';
OE1.Cells[2,8].Value:='№ Пост';
OE1.Cells[2,9].Value:='РасходТопливо';
OE1.Cells[2,10].Value:='ТипКаробки';
OE1.Cells[2,11].Value:='Производитель';
OE1.Cells[2,12].Value:='Объём';


istr:=3;                           // Номер
RADM.ADOQueryOtch.First;               //Перейти на первую запись
while not RADM.ADOQueryOtch.Eof do  // Пока не будет достигнут конец файла
begin
...
OE1.Range['A3:L'+IntToStr(istr+1)].Select;   // Выделить ячейки
...
procedure TRAKa.BitBtn4Click(Sender: TObject);     // Диалоговое окно - Выход из программы.
begin
...
procedure TRAKa.BitBtn5Click(Sender: TObject);     // Диалоговое окно - Закрытие формы программы.
begin
...
procedure TRAKa.CheckBox1Click(Sender: TObject);       // Переписать код фильтрафии! Написать функцию для кнопки -  "Фильтр",
                                                      //  сразу для всех компонентов чекбокс
begin
...
procedure TRAKa.Edit10Change(Sender: TObject);      // Поиск по таблицу AVTO.
