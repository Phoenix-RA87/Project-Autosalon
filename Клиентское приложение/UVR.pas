...
begin
 // №_Дог, дада Рекламации - проверка на ввод данных.

  if Length(Trim(LabeledEdit2.Text)) = 0 then
   begin
     MessageDlg('Вы не указали Неисправность!',mtError,[mbOk],0);
     Exit     //Выход
  end;

  if Length(Trim(LabeledEdit2.Text)) = 0 then
   begin
     MessageDlg('Вы не указали Результат!',mtError,[mbOk],0);
     Exit     //Выход
  end;

  if Length(Trim(LabeledEdit3.Text)) = 0 then
   begin
     MessageDlg('Вы не указали Сумму Убытков!',mtError,[mbOk],0);
     Exit     //Выход
  end;
...
ShowMessage('Новая рекламация успешно добавлена.'); // Диалоговое окно.
 ...
 DBGrid1.Columns.Items[0].Title.Caption :='№ Реклам';
  DBGrid1.Columns.Items[1].Title.Caption :='ДатаРеклам';
   DBGrid1.Columns.Items[2].Title.Caption :='№ Дог';
    DBGrid1.Columns.Items[3].Title.Caption :='Неисправность';
     DBGrid1.Columns.Items[4].Title.Caption :='Результат';
      DBGrid1.Columns.Items[5].Title.Caption :='СуммаУбыт';
end;

procedure TRAVR.BitBtn4Click(Sender: TObject);   // Диалоговое окно - Закрытие формы программы.
begin
...
procedure TRAVR.BitBtn5Click(Sender: TObject);  // Диалоговое окно - Выход из программы.
begin
