...
if Length(Trim(LabeledEdit4.Text)) = 0 then
  begin
    MessageDlg('Вы не ввели БазЦену.',mtError,[mbOk],0);
    Exit     //Выход
  end;

if Length(Trim(LabeledEdit5.Text)) = 0 then
  begin
    MessageDlg('Вы не ввели Мощность.',mtError,[mbOk],0);
    Exit     //Выход
  end;

if Length(Trim(LabeledEdit3.Text)) = 0 then
  begin
    MessageDlg('Вы не ввели № Сертиф.',mtError,[mbOk],0);
    Exit     //Выход
  end;

if Length(Trim(LabeledEdit9.Text)) = 0 then
  begin
    MessageDlg('Вы не ввели РасходТоплива.',mtError,[mbOk],0);
    Exit     //Выход
  end;

if Length(Trim(LabeledEdit6.Text)) = 0 then
  begin
    MessageDlg('Вы не ввели Объём.',mtError,[mbOk],0);
    Exit     //Выход
  end;
...
ShowMessage('Новый автомобиль успешно добавлен.'); // Диалоговое окно.

RADM.ADOQueryAVTO.Close;
RADM.ADOQueryAVTO.Open;
end;

procedure TRAVA.BitBtn4Click(Sender: TObject);   // Диалоговое окно - Закрытие формы программы.
begin
...
procedure TRAVA.BitBtn5Click(Sender: TObject); // Диалоговое окно - Выход из программы.
begin
...
DBGrid1.Columns.Items[0].Title.Caption :='КодТовара';
 DBGrid1.Columns.Items[1].Title.Caption :='Марка';
  DBGrid1.Columns.Items[2].Title.Caption :='Модель';
   DBGrid1.Columns.Items[3].Title.Caption :='ДатаПост';
     ...
     DBGrid1.Columns.Items[5].Title.Caption :='БазЦена';
      DBGrid1.Columns.Items[6].Title.Caption :='Мощность';
       DBGrid1.Columns.Items[7].Title.Caption :='№ Пост';
         DBGrid1.Columns.Items[8].Title.Caption :='РасходТоплива';
          DBGrid1.Columns.Items[9].Title.Caption :='ТипКоробки';
